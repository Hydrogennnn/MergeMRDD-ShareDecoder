...
def main():
    ...
    for ms in range(0,10):
        for noise_prob in range(0,10):
            print(f"[Evaluation on {ms / 10}modal missing {noise_prob} sp noise]")
            generate_missing(m_ratio=ms/10,tot_nums=len(train_set), dataset_name=config.dataset.name, num_views=config.views)
            mask_train_set = get_mask_train_dataset(config, val_transformations, m_ratio=ms/10)
            ...
            consistency, vspecific, concate, all_concate, labels = extract_features(mask_train_dataloader, model, device, noise_prob/10)
            print('eval on consist...')
            cluster_acc, _, _, cls_acc, _, _ = report(run_times, n_clusters, need_classification, labels, consistency)
            eval_res["cluster_mean"].append(np.mean(cluster_acc))
            eval_res["cluster_std"].append(np.std(cluster_acc))
            eval_res["cls_mean"].append(np.mean(cls_acc))
            eval_res["cls_std"].append(np.std(cls_acc))
